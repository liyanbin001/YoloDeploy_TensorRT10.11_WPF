...
internal static class EngineCacheManager
{
    ...
    // Intentionally the same folder as the existing WPF application.
        ...
        int workspaceMiB)
    {
        ...
        string stem = SanitizeToken(
            Path.GetFileNameWithoutExtension(onnxPath));

        string gpuToken = SanitizeToken(gpu.Name);
        string hashToken =
            onnxSha256[..Math.Min(16, onnxSha256.Length)];

        string precisionToken =
            SanitizeToken(precision.ToLowerInvariant());
        ...
        out string reason)
    {
        ...
        try
        {
            string json = File.ReadAllText(descriptor.MetadataPath);
            ...
            if (!string.Equals(
                    ...
                    gpu.Name,
                    StringComparison.OrdinalIgnoreCase) ||
                metadata.ComputeCapabilityMajor != gpu.ComputeCapabilityMajor ||
                metadata.ComputeCapabilityMinor != gpu.ComputeCapabilityMinor ||
                metadata.MultiProcessorCount != gpu.MultiProcessorCount)
            {
                reason = "GPU 配置不同";
                return false;
            }
            ...
            long currentLength =
                new FileInfo(descriptor.EnginePath).Length;
    ...
    internal static void WriteMetadata(
        ...
        string buildLog)
    {
        FileInfo engineFile = new(descriptor.EnginePath);
        ...
        var metadata = new EngineCacheMetadata
        {
            SchemaVersion = CurrentSchemaVersion,
            SourceOnnxPath = descriptor.OnnxPath,
            OnnxSha256 = descriptor.OnnxSha256,
            Precision = descriptor.Precision,
            InputWidth = descriptor.InputWidth,
            InputHeight = descriptor.InputHeight,
            WorkspaceMiB = descriptor.WorkspaceMiB,

            GpuName = gpu.Name,
            ComputeCapabilityMajor = gpu.ComputeCapabilityMajor,
            ComputeCapabilityMinor = gpu.ComputeCapabilityMinor,
            MultiProcessorCount = gpu.MultiProcessorCount,
            TotalGlobalMemoryBytes = gpu.TotalGlobalMemoryBytes,

            CudaRuntimeVersion = gpu.CudaRuntimeVersion,
            CudaDriverVersion = gpu.CudaDriverVersion,
            TensorRtMajor = gpu.TensorRtMajor,
            TensorRtMinor = gpu.TensorRtMinor,
            TensorRtPatch = gpu.TensorRtPatch,
            TensorRtBuild = gpu.TensorRtBuild,

            EngineLengthBytes = engineFile.Length,
